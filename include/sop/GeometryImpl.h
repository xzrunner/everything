...
{
public:
    GeometryImpl(const ur::Device& dev, GeoAdaptor::Type type);
    ...
    void UpdateByBrush(const ur::Device& dev, const model::BrushModel& brush_model);
    void StoreBrush(std::unique_ptr<model::BrushModel>& brush_model);

    void UpdateByAttr(const ur::Device& dev);
